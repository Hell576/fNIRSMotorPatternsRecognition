...
def repl_23to01labelMap(sourceLabel, destLabels:list):
...
def repl_12to2301Map(sourceLabel, destLabels: list):
    if (sourceLabel == 0):
        return destLabels[1]
    elif (sourceLabel == 1):
        return destLabels[0]
...
ds = tf.data.Dataset.load('Full12DsLSTM') #'Full23DsLSTM' #ConcatDS6-
#train_ds = tf.data.Dataset.load('TrainHugeDS')
#valid_ds = tf.data.Dataset.load('ValidHugeDS')
#test_ds = tf.data.Dataset.load('TestHugeDS')
...
ds = ds.map(lambda x,y: (x, repl_12to2301Map(y, [0, 1])))
...
#also Dataset inside print function
...
weights_filepath = 'LSTM_weights.keras'
model.save(weights_filepath)
#model.load_weights(weights_filepath, skip_mismatch=False)
