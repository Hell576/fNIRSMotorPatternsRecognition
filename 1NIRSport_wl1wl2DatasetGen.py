...
#RASPREDELENIE PODELENNYX SOBYTIY PO Tensoram
...
#need to 'pip install pypiwin32 v223' first
...
def gen_ds_script():
    ...
    print('Если все файлы - тогда жмите Enter')
    needCount = input()
    if needCount != '':
        ...
        print('Поэтому выбрано максимальное кол-во:', totalCount)
    ...
    print('Если никакое событие не нужно исключать, просто нажмите Enter')
    classesToReduce = input().split(sep=' ')

    if classesToReduce != ['']:
        classesToReduce = sorted(map(int, classesToReduce))
    else: classesToReduce = []

    print('Пожалуйста, подождите: генерируется датасет. Не обращайте внимания на вывод еensorflow, Ждите русский текст')
    ...
    addname = input('Укажите добавочное имя датасету по желанию или нажмите enter, чтобы пропустить\n')

    fulldsnamepath = 'FullNIRS' + joined_str + 'tfDataset' + addname
    ds.save(fulldsnamepath)
    sampdsnamepath = 'sampleNIRS' + joined_str + 'tfDataset' + addname
...
if __name__ == "__main__":
    ...
    print('Программа делает из каждой тройки файлов *.wl1,*.wl2,*.hdr датасет для tensorflow.\n Она собирает все данные из файлов wl1+wl2(объединяет столбцы) и группирует по отдельным событиям из файла hdr(имя файлов wl1, wl2 = имя файла hdr).\n На выходе получается датасет из элементов структуры типа "Матрица->номер_события"')
    while (True):
        ...
        if choose == '1':
            ...
            result_ds.save('concatNIRS_DS' + addname)
            print('Сохранено как concatNIRS_DS')
